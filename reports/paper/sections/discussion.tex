...
\paragraph{A single policy recommendation.}
The geography, lever and fairness results compose into one concrete, if
associational, recommendation. \emph{Target by place, not just by income.} Because the
SES gradient is flat, means-tested transfers alone miss most at-risk students, whereas the
North and rural bands carry a load $7$ to $10$ points above their comparators (and the
public sector $\sim$25 points above the small private one), so geography is where a fixed
remediation budget reaches the most children. \emph{Match the instrument to the place.}
Math anxiety is the one soft lever with measurable reach ($-5.1$ points, 95\% CI
$[4.5,5.7]$), but its reach is concentrated in urban schools and thins out in the rural
band; so anxiety-oriented pedagogy (formative low-stakes assessment, teacher support) is a
sensible \emph{urban} first move, while the rural and northern floor, where no soft lever
moves the needle and even lifting every endowment leaves a majority at risk, calls for
structural investment (teaching capacity, learning time, school resources) rather than
psychosocial programmes. \emph{And screen to widen support, not to ration it.} Deploy the
calibrated screener with the group-specific thresholds that equalise false-positive rates
across SES, so the poorest quintile is not disproportionately flagged, and treat a flag as
a route into these place-matched supports. The recommendation is deliberately singular:
target the North, rural and public bands; lead with anxiety programmes in urban schools
and structural investment in rural ones; and use an equity-corrected screener to direct
the effort.
