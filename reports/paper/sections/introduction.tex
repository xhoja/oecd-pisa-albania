...
socioeconomic composition, certify the new plateau as a genuine data ceiling through
three independent routes, and show it both generalises across nine systems (its size
tracking each system's between-school inequality) and withstands richer process data
that only moves it through a signal endogenous to the test; (iii) characterise the
crisis as broad-based and shared with regional peers, and test a single-region
earthquake explanation with a difference-in-differences that instead identifies the
collapse as national; and (iv) connect these statistical findings to Albania's economic
and structural context, while delivering a calibrated, interpretable, coverage-bounded
and fairness-mitigable risk screener that we argue should not be read causally.
