...
\subsection{Geographic equity and lever ranking}
Two descriptive-to-prescriptive analyses target the crisis rather than just measure it.
For \emph{geographic equity}, the sampling stratum label is parsed into a
region-band $\times$ urbanicity $\times$ sector cell for the 2015/2018/2022 cycles; the
weighted below-Level-2 rate and its design-based (replicate-weight and plausible-value)
standard error are computed per cell, and each between-group gap is estimated on the
contrast itself so its standard error is exact under the design rather than inferred from
two marginal intervals. For the \emph{lever ranking}, the fitted school-context model's
features are partitioned into fixed endowments and malleable levers; each lever is nudged
half a training-standard-deviation in the direction that lowers the smooth mean predicted
probability (its engineered interactions and school-mean aggregate moving with it), and
levers are ranked by the resulting change in the weighted population predicted at-risk
rate. Each lever effect carries a population-bootstrap 95\% confidence interval (400
resamples of the students with the fitted model held fixed, so the interval captures
finite-cohort sampling uncertainty but not model-fit uncertainty). A fixed-versus-malleable
bundle contrast sizes how far actionable levers reach before the structural floor. Like the
policy simulations these are model-associational contrasts, not causal effects.

\subsection{Reproducibility and engineering}
Logic lives in unit-tested modules (189 tests on synthetic fixtures, covering the
