...
from src.models.levers import lever_ceiling, rank_levers
...
def _plot(rank: pd.DataFrame, base: float) -> None:
    ...
    ax.barh(r["description"], r["reduction_pp"], color=PALETTE["blue"],
            edgecolor="white")
    # labels always to the right of zero so they never hit the y-axis label gutter
...
def main() -> None:
    ...
    print("=== NATIONAL MALLEABLE-LEVER RANKING (0.5-SD beneficial nudge) ===")
    print(rank[["lever", "description", "direction", "delta_at_risk_rate",
                "matches_expected_sign"]].to_string(index=False))
